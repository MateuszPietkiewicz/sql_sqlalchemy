...
# kwota wpływów w podziale na pracowników
...
SELECT customer_id, SUM(amount) Amount, DATE_FORMAT(payment_date, '%Y-%m') AS PymentMonth
FROM payment
GROUP BY customer_id, PymentMonth
ORDER BY customer_id, Amount DESC
LIMIT 500;

SELECT staff_id, SUM(amount) Amount, DATE_FORMAT(payment_date, '%Y-%m') AS PymentMonth
FROM payment
...
CREATE OR REPLACE VIEW payment_raport
AS
SELECT cust.first_name,
       ...
       MAX(p.payment_date) AS CustimerLAstPayment
FROM customer cust
...
# test
SELECT (SELECT SUM(CustomerTotal) FROM payment_raport) = (SELECT SUM(amount) FROM payment);
...
# to samo tylko CTE (common table expression)
...
SELECT tfa.title, tfa.ActorCount, tfp.Amount, tfr.RentalCount
...
SELECT p.customer_id, p.staff_id, SUM(p.amount) Total
FROM payment p
