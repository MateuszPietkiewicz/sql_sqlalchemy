SELECT cust.customer_id,
       ...
       DENSE_RANK() OVER (ORDER BY SUM(pt.amount) DESC) AS customer_rank,
...
# ile klient wydal miesiecznie + roznica wzgledem poprzedniego miesiaca


SELECT cust.customer_id,
       DATE_FORMAT(pt.payment_date, '%Y-%m') AS month,
       SUM(pt.amount) AS amount_month,
       SUM(pt.amount) -LAG(SUM(pt.amount)) OVER (PARTITION BY cust.customer_id ORDER BY DATE_FORMAT(pt.payment_date, '%Y-%m')) AS diff_from_prev_month
FROM customer cust
JOIN payment pt ON cust.customer_id = pt.customer_id
GROUP BY cust.customer_id, month
LIMIT 500;

# top 3 filmy z kazdej kategori ( category_name, film_title, rental_count, ranking )
...
WITH cte AS (
        SELECT ADDDATE('2000-01-01', (ROW_NUMBER() OVER ()) - 1) AS date

        FROM payment
        LIMIT 11323)
SELECT date,
       EXTRACT(year from date) AS dateYear,
       EXTRACT(year from date) AS dateMonth,
       EXTRACT(year from date) AS dateDay,
...
# Napisz kwerende, która stworzy ranking na podstawie średniego ratingu z filmów, w którym grali
...
SELECT actor_id,
       ...
       SUM(actor_payload) OVER _order AS Sum,
...
SELECT actor_id,
       actor_payload,
       (ROW_NUMBER() over (payload) / COUNT(*) OVER ()) * 100 AS count_percent,
